...
package sk.navigation.demo.smoothnavigation.smoothcore;
...
public class FragmentNavigator extends Navigator<FragmentNavigator.Destination> {
    ...
    public boolean popBackStack() {
        ...
        //Tip 2: Increase fault tolerance to deal with incorrectly-timed jumps in the case of nested sub-fragments
        ...
        Log.e(TAG, "popBackStack, removeIndex:" + removeIndex);
    ...
    public NavDestination navigate(@NonNull Destination destination, @Nullable Bundle args,
        ...
        // Mark0: origin fragment replace logic
...
//        ft.setPrimaryNavigationFragment(frag);
        ...
        //Mark1: Solve the unexpected display of popUpToInclusive under the add hide solution
        //Mark2: Increase fault tolerance to deal with incorrectly-timed jumps in the case of nested sub-fragments
        Log.d("Nav", " --mBackStack.size:" + mBackStack.size()
                + " getFragments().size():" + mFragmentManager.getFragments().size());

        // Mark0: new fragment add logic
        if (mBackStack.size() > 0 && mFragmentManager.getFragments().size() > 0) {
            ...
            if (isSingleTopReplacement) {
                ...
                ft.setMaxLifecycle(hideFrag, Lifecycle.State.STARTED);
                ft.setMaxLifecycle(hideFrag, Lifecycle.State.RESUMED);
            } else {
                ft.hide(hideFrag);
                ft.add(mContainerId, frag);
                // Mark3: move lifeCycle sate to STARTED for fragment those are hide,
                // so that onPause/onResume lifeCycleCallback will be called.
                ft.setMaxLifecycle(hideFrag, Lifecycle.State.STARTED);
                ft.setPrimaryNavigationFragment(frag);
            }
        } else {
